...
export function StakeModal({
  event: { pool, startTime, endTime, id, rewardAddress, bonusRewardAddress },
  closeHandler,
}: {
  event: {
    pool: string
  ...
  const positionsForStake = useMemo(() => {
    ...
    return positionsForPool.filter((position) => position.pool === pool && !position.stakedInIncentive)
  }, [positionsForPool])
  ...
  const NFTsForStake = useMemo(
    () => filterNFTs((v) => v.transfered && !v.stakedInIncentive),
    [selectedNFT, submitState]
  )
      ...
      for (const row of chunkedPositions) {
        ...
        for (const position of row) {
          if (position.tokenId === stakedHash.id) {
            position.stakedInIncentive = true
            position.transfered = true
            position.approved = true
            setSelectedNFT((old) => ({
              ...old,
              stakedInIncentive: true,
              transfered: true,
              approved: true,
            }))
          }
                  ...
                  {row.map((el, j) => (
                        ...
                        if (e.target.tagName !== 'A' && !submitLoader) {
                          setSelectedNFT((old) =>
                            old && old.tokenId === el.tokenId
                              ? null
                              : {
                                  stakedInIncentive: el.stakedInIncentive,
                                  tokenId: el.tokenId,
                    ...
                    >
                      ...
                      <NFTPositionDescription>
                        ...
                        <NFTPositionLink href={`https://algebra.exchange/#/pool/${el.tokenId}`} target="_blank">
                          View position
                        </NFTPositionLink>
                      </NFTPositionDescription>
